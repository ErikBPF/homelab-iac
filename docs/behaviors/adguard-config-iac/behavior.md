...
**Status:** Implemented — owned provider lifecycle green
...
The Registry provider is the required production lane. Setting
...
Provider `ErikBPF/adguardhome` 0.1.8 passes the complete disposable lifecycle when
...
drift; it was not a valid admission failure. The owned provider is the accepted
