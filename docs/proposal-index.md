...
|---|---|---:|---:|---|---|
| [Repository responsibility hardening](proposals/2026-08-16-repository-responsibility-hardening.md) | In progress; GitOps containment, boundary docs, and pinned IaC/Telstar execution deployed | High | High | Reconcile Harbor admin credential under an approved procedure, create the pull-only robot, sync Airflow at the reviewed SHA, and compare one pinned drift plan. | `homelab` gates; `homelab-gitops` Applications and Airflow; `homelab-iac` command revision, fleet pin, ESO contract, and docs; `desktop-nixos` pinned schedulers; `servarr` Harbor entrypoint, fleet pin, and secret registry |
