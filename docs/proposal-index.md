...
| [Fleet Wazuh SIEM integration](proposals/2026-07-26-fleet-wazuh-siem-integration.md) | In progress on Kepler; fresh UniFi CEF-to-index proof and disk watermarks deployed | High | High | Create a consistent index snapshot path and prove isolated restore before one canary agent. | `servarr` security stack, `desktop-nixos` Kepler firewall/diagnostics, UniFi CyberSecure/syslog |
