...
|---|---|---:|---:|---|---|
| [Repository responsibility hardening](proposals/2026-08-16-repository-responsibility-hardening.md) | In progress; producer leaves and exact activation landed; P2 evidence, immutable Servarr publication, and rollout remain | High | High | Complete approved Harbor recovery/Airflow sync, authorize one immutable Servarr artifact channel, then run the Discovery canary and retain drift/health/rollback evidence. | `homelab` gates; `homelab-gitops` Applications and Airflow; `homelab-iac` pinned command and fail-closed retry; `desktop-nixos` pinned schedulers, exact Servarr activation, and Kindle routing; `servarr` immutable Harbor/Kindle producer files and secret registry |
