...
| [Gemini persistent code stack](proposals/2026-07-23-gemini-persistent-code-stack.md) | Partially implemented; Galaxy SSH to Gemini and Endeavour, Endeavour sync, and the persistent Gemini Homelab workspace are deployed | High | Medium | Prove Galaxy/Termius allow/deny and detach/reattach behavior, then capture native agent integration restore evidence. | `homelab-iac` Tailscale ACL/tests; `desktop-nixos` Gemini/Endeavour SSH, Syncthing, and Herdr modules; operator-owned Termius key |
