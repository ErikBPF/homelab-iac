...
|---|---|---:|---:|---|---|
| [Repository responsibility hardening](proposals/2026-08-16-repository-responsibility-hardening.md) | In progress; P0–P2 and the exact Servarr Discovery canary are complete; immutable publication and credential-gated recovery remain | High | High | Complete approved Harbor recovery/Airflow sync, authorize one immutable Servarr artifact channel, and separately authorize Telstar lock recovery. | `homelab` gates and evidence; `homelab-gitops` Applications and Airflow; `homelab-iac` pinned command, serialized drift, and fail-closed retry; `desktop-nixos` pinned schedulers, exact Servarr activation, and Kindle routing; `servarr` immutable Harbor/Kindle producer files and secret registry |
