...
|---|---|---:|---:|---|---|
| [Alert reliability improvements](proposals/2026-08-14-alert-reliability-improvements.md) | In progress; implementation merged and cache root cause classified; retry, deployment, and live proof pending | High | Medium | Run the manual cache retry when Orion capacity permits, then deploy Alloy-after-Docker before the Servarr telemetry/workload rule split. | `desktop-nixos` Alloy ordering and diagnostics; `servarr` Grafana rules, tests, and runbook; `homelab` gates and evidence |
| [Endeavour LUKS authentication and convenience unlock](proposals/2026-08-12-endeavour-luks-authentication.md) | Paused; slots `0` and `1` verified, boot proof and durable header backup pending | Medium | High | Unlock once with slot `1`, then create an encrypted off-device header backup and choose FIDO2 or dual-passphrase steady state. | `desktop-nixos` Endeavour disk/initrd and Hyprlock/fprintd configuration; operator-owned LUKS2 header state |
...
| [Fleet container placement and SRP](proposals/2026-07-11-fleet-container-placement-srp.md) | Proposed; live placement needs re-audit and its retired PocketID slice is obsolete | Medium | High | Re-audit live placement before moving workloads; do not implement the retired PocketID extraction. | `servarr` compose stacks, `desktop-nixos` orchestration, `homelab-gitops` workloads |
